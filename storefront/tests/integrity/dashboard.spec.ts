...
import { getDashboardOrigin, getDashboardUrl } from '../../src/lib/dashboard'
...
function withDashboardUrl(value: string | undefined, assertion: () => void) {
  const original = process.env.NEXT_PUBLIC_DASHBOARD_URL
  ...
  try {
    ...
    if (original === undefined) {
      ...
      process.env.NEXT_PUBLIC_DASHBOARD_URL = original
    }
  }
}
...
test.describe('Dashboard route — smoke', () => {
  ...
  test('shows fallback when NEXT_PUBLIC_DASHBOARD_URL is missing', () => {
    ...
    expect(content).toContain(NEXUS_ADMIN_LOGIN_URL)
  })
  ...
  test('/dashboard renders the configured iframe or safe fallback with matching CSP', async ({ page }) => {
    ...
    if (dashboardUrl && dashboardOrigin) {
      await expect(page.locator('iframe[title="GrowMedica Dashboard"]')).toHaveAttribute('src', dashboardUrl)
      await expect(page.getByTestId('dashboard-dev-nexus-link')).toHaveAttribute('href', NEXUS_ADMIN_LOGIN_URL)
      expect(csp).toBe(`frame-src 'self' ${dashboardOrigin}`)
    } else {
      await expect(page.getByTestId('dashboard-unconfigured')).toBeVisible()
      await expect(page.getByTestId('dashboard-nexus-direct-link')).toHaveAttribute('href', NEXUS_ADMIN_LOGIN_URL)
      expect(csp).toBe("frame-src 'self'")
    }
  })
...
test.describe('Dashboard URL helper', () => {
  test('normalizes safe dashboard URLs and derives the CSP origin', () => {
    withDashboardUrl('  https://growmedica-nexus.lovable.app/admin  ', () => {
      expect(getDashboardUrl()).toBe('https://growmedica-nexus.lovable.app/admin')
      expect(getDashboardOrigin()).toBe('https://growmedica-nexus.lovable.app')
    })

    withDashboardUrl('http://localhost:8080/admin', () => {
      expect(getDashboardUrl()).toBe('http://localhost:8080/admin')
      expect(getDashboardOrigin()).toBe('http://localhost:8080')
    })
  })

  test('rejects empty, malformed, and non-http dashboard URLs', () => {
    for (const value of [undefined, '', 'notaurl', 'javascript:alert(1)', 'ftp://example.com/admin']) {
